...
diag: $(ASM_MODULES) $(DIAG_MODULES)
	...
	# testable and assertable), its own zdisp_read_line (a canned-
	# input test double, so sread stays bare-metal testable too), and
	# its own zdisp_save_game/zdisp_restore_game (an in-memory-buffer
	# test double, so save/restore stay bare-metal testable without
	# touching a real file) -- lib/zdispemit.prg/lib/zdispread.prg/
	# lib/zdispsave.prg's real, kernel-backed implementations of those
	# same names are deliberately left out of this link (also
	# sidestepping lib/zterm.prg/lib/zinputl.prg, which the real
