...
# diag/zdiag_main is the ELF-DOS front end for zdiag.asm's checks against
# the resident memory/stack primitives (see docs/ARCHITECTURE.md's
# "diagnostic dispatch loop" step) -- run it on hardware or under an
...
clean:
	...
	rm -f diag/zdiag diag/zdiag.lkb